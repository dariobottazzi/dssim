...
class BaseService(object):
    ...
    def log(self, logged_data):
        """
        makes it possible to log data produced by a service. The string is logged only if the public attribute verbose
        is set to True. Otherwise the data is not logged.
    ...
    def handle_indication(self, msg):
        """
        This method invoked upon receiving a message by the node indication method. The message is generated by another
        service running over the same node.
